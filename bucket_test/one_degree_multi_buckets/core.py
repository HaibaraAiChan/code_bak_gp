...
def invoke_udf_reduce(graph, func, msgdata, degree, num_split, step, *, orig_nid=None, ):
    ...
    if degree >=1 :
        ...
        unique_degs, bucketor = _bucketing(degs, degree=-1, num_split=-1, step=-1)
    #--------------------------------------------------------------------new added end
    # degree bucketing
...
def _bucketing(val, degree, num_split, step): 
    ...
    if F.equal(degree, -1): # all degree bucket
        ...
        if (step==-1) or (num_split==-1): # singe degree bucket
            ...
            #     N = math.ceil(len(tmp)/num_split)
            #     global_split_bkt_idx.append(tmp[:N])
            #     for i in range(1,num_split-1):
            #         global_split_bkt_idx.append(tmp[i*N:((i+1)*N)])
            #     global_split_bkt_idx.append(tmp[(num_split-1)*N:])
            #     bkt_idx = global_split_bkt_idx[0]
            # elif step > 0:
            #     if step <= len(global_split_bkt_idx)-1:
            #         bkt_idx = global_split_bkt_idx[step]
            unique_val = np.asarray(list([degree.item()]))
