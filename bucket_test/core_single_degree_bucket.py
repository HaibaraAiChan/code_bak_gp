...
def invoke_udf_reduce(graph, func, msgdata, degree, *, orig_nid=None, ):
    ...
    #-------------------------------------------------------------------new added-start
    if degree :
        unique_degs, bucketor = _bucketing(degs, degree=degree)
    else:
        unique_degs, bucketor = _bucketing(degs, degree=None)
    #--------------------------------------------------------------------new added end
    # degree bucketing
...
def _bucketing(val, degree): 
    ...
    # #---------------------------------------------------------------orignal part
    #--------------------------------------------------------------*-replaced part
    if F.equal(degree, -1): # all degree bucket
        for v in unique_val:
            eqidx = F.nonzero_1d(F.equal(sorted_val, v))
            bkt_idx.append(F.gather_row(idx, eqidx))
    
    else:  # single degree bucket
        v = degree.item()
        eqidx = F.nonzero_1d(F.equal(sorted_val, v))
        bkt_idx.append(F.gather_row(idx, eqidx))
...
def message_passing(g, mfunc, rfunc, afunc, degree):   # new code
    ...
    if (
        ...
        if is_builtin(rfunc):
            ...
            ndata = invoke_udf_reduce(g, rfunc, msgdata, degree, orig_nid=orig_nid) # new
    # apply phase
